...
class ReportService
{
    ...
    {
        // Mengambil semua Sales yang aktif untuk dibuatkan peringkat.
        $salesUsers = User::role('Sales')->where('is_active', true)->get();
    ...
    public function getForecastData(): array
    {
        ...
        for ($i = 5; $i >= -6; $i--) {
            ...
            // Menghitung pendapatan nyata dari Deal yang sudah WON pada bulan tersebut.
            $actual = Deal::won()
                ->whereYear('closed_at', $date->year)
                ->whereMonth('closed_at', $date->month)
                ->sum('value');

            // Menghitung perkiraan pendapatan dari Deal OPEN berdasarkan kemungkinan stage.
            $projected = Deal::open()
                ->with('pipelineStage')
        ...
        // Menghitung perkiraan hasil terbaik jika semua Deal OPEN berhasil.
        ...
        $worstCase = $closingStageId
            ? Deal::open()->where('pipeline_stage_id', $closingStageId)->sum('value')
            : 0;
