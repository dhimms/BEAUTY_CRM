...
@section('breadcrumb')
    ...
    <li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></li>
    <li><a href="{{ route('admin.leads.show', $lead) }}" class="hover:text-charcoal-900 transition-colors">{{ $lead->name }}</a></li>
    <li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></li>
    <li class="text-charcoal-900 font-medium">Edit Lead</li>
@endsection
@section('page-header', 'Edit Lead')

@section('content')
<x-card class="max-w-4xl">
    <form action="{{ route('admin.leads.update', $lead) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $lead->name) }}" required class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $lead->phone) }}" required class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $lead->email) }}" class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3" class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">{{ old('address', $lead->address) }}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="lead_source_id" id="lead_source_id" required class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">
                        ...
                        @foreach($sources as $source)
                            <option value="{{ $source->id }}" {{ old('lead_source_id', $lead->lead_source_id) == $source->id ? 'selected' : '' }}>{{ $source->icon }} {{ $source->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="assigned_to" id="assigned_to" class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">
                        ...
                        @foreach($salesList as $sales)
                            <option value="{{ $sales->id }}" {{ old('assigned_to', $lead->assigned_to) == $sales->id ? 'selected' : '' }}>{{ $sales->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="qualification" id="qualification" class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">
                            ...
                            <option value="qualified" {{ old('qualification', $lead->qualification) == 'qualified' ? 'selected' : '' }}>Qualified</option>
                            <option value="unqualified" {{ old('qualification', $lead->qualification) == 'unqualified' ? 'selected' : '' }}>Unqualified</option>
                            <option value="not_fit" {{ old('qualification', $lead->qualification) == 'not_fit' ? 'selected' : '' }}>Not Fit</option>
                        </select>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4" class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm">{{ old('notes', $lead->notes) }}</textarea>
