...
@endsection
@section('page-header', 'Add Lead Source')

@section('content')
<x-card class="max-w-2xl">
    <form action="{{ route('admin.lead-sources.store') }}" method="POST" class="space-y-6">
        @csrf

        <div>
            <label for="name" class="block text-sm font-medium text-charcoal-900 mb-1">Name <span class="text-rose-500">*</span></label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm" placeholder="e.g., Facebook Ads, Instagram, Walk-in">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="icon" id="icon" value="{{ old('icon') }}" class="w-full px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm" placeholder="e.g., 📘, 📸, 🚶">
            ...
            <div>
                ...
                <div class="flex gap-2">
                    <input type="color" name="color" id="color" value="{{ old('color', '#F43F5E') }}" class="h-10 w-14 rounded-xl cursor-pointer bg-charcoal-50 border border-charcoal-200">
                    <input type="text" value="{{ old('color', '#F43F5E') }}" oninput="document.getElementById('color').value = this.value" class="flex-1 px-4 py-2 bg-charcoal-50 border border-charcoal-200 rounded-xl focus:ring-rose-500 focus:border-rose-500 sm:text-sm" placeholder="#F43F5E">
                </div>
        ...
        <div class="pt-4 border-t border-charcoal-200 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-rose-600 rounded-xl hover:bg-rose-700">Save Lead Source</button>
        </div>
    </form>
</x-card>
@endsection
